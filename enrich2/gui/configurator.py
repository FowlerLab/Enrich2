...
class Configurator(tk.Tk):
    ...
    def create_main_frame(self):
        ...
        main = six.moves.tkinter_ttk.Frame(self, padding=(3, 3, 12, 12))
        main.rowconfigure(0, weight=1)
        main.columnconfigure(0, weight=1)
        main.grid(row=0, column=0, sticky="nsew")

        # Frame for the Treeview and its scrollbars
        tree_frame = six.moves.tkinter_ttk.Frame(main, padding=(3, 3, 12, 12))
        ...
        self.treeview = six.moves.tkinter_ttk.Treeview(tree_frame)
        ...
        button_frame = six.moves.tkinter_ttk.Frame(main, padding=(3, 3, 12, 12))
        button_frame.grid(row=1, column=0)
        new_button = six.moves.tkinter_ttk.Button(button_frame, text="New...", command=self.new_button_press)
        new_button.grid(row=0, column=0)
        edit_button = six.moves.tkinter_ttk.Button(button_frame, text="Edit...", command=self.edit_button_press)
        edit_button.grid(row=0, column=1)
        delete_button = six.moves.tkinter_ttk.Button(button_frame, text="Delete", command=self.delete_button_press)
        delete_button.grid(row=0, column=2)

        # Frame for Analysis Options
        options_frame = six.moves.tkinter_ttk.Frame(main, padding=(3, 3, 12, 12))
        options_frame.grid(row=0, column=1, rowspan=2, sticky="nsew")

        row = 0
        heading = six.moves.tkinter_ttk.Label(options_frame, text="Analysis Options")
        ...
        other_heading = six.moves.tkinter_ttk.Label(options_frame, text="Other Options")
        other_heading.grid(column=0, row=row)
        row += 1

        # force recalculate
        force_recalculate = six.moves.tkinter_ttk.Checkbutton(options_frame, text="Force Recalculation", variable=self.force_recalculate)
        force_recalculate.grid(column=0, row=row, sticky="w")
        row += 1

        # component outliers
        component_outliers = six.moves.tkinter_ttk.Checkbutton(options_frame, text="Component Outlier Statistics", variable=self.component_outliers)
        component_outliers.grid(column=0, row=row, sticky="w")
        row += 1

        # make plots
        plots_requested = six.moves.tkinter_ttk.Checkbutton(options_frame, text="Make Plots", variable=self.plots_requested)
        plots_requested.grid(column=0, row=row, sticky="w")
        plots_requested.invoke()
        row += 1

        # write tsv
        tsv_requested = six.moves.tkinter_ttk.Checkbutton(options_frame, text="Write TSV Files", variable=self.tsv_requested)
        tsv_requested.grid(column=0, row=row, sticky="w")
        tsv_requested.invoke()
        row += 1

        go_button = six.moves.tkinter_ttk.Button(options_frame, text="Run Analysis", command=self.go_button_press)
        go_button.grid(column=0, row=row, sticky="sew")


    def go_button_press(self):
        if self.root_element is None:
            six.moves.tkinter_messagebox.showwarning("", "No experimental design specified.")
        else:
    ...
    def new_button_press(self):
        if self.treeview.focus() == "" and self.root_element is not None:
            six.moves.tkinter_messagebox.showwarning(None, "No parent element selected.")
        else: 
    ...
    def edit_button_press(self):
        if self.treeview.focus() == "":
            six.moves.tkinter_messagebox.showwarning(None, "No element selected.")
        else:
    ...
    def menu_open(self):
        fname = six.moves.tkinter_filedialog.askopenfilename()
        if len(fname) > 0:  # file was selected
            try:
                with open(fname, "rU") as handle:
                    cfg = json.load(handle)
            except ValueError:
                six.moves.tkinter_messagebox.showerror(None, "Failed to parse config file.")
            except IOError:
                six.moves.tkinter_messagebox.showerror(None, "Could not read config file.")
            else:
                if is_experiment(cfg):
                    ...
                    six.moves.tkinter_messagebox.showerror(None, "Unrecognized config format.")
                    return
                obj.output_dir_override = False
                try:
                    obj.configure(cfg)
                except Exception as e:
                    six.moves.tkinter_messagebox.showerror(None, "Failed to process config file:\n{}".format(e))
                else:
    ...
    def menu_save(self):
        if len(self.cfg_file_name.get()) == 0:
            ...
            six.moves.tkinter_messagebox.showwarning(None, "Cannot save empty configuration.")
        else:
            try:
                with open(self.cfg_file_name.get(), "w") as handle:
                    write_json(self.root_element.serialize(), handle)
            except IOError:
                six.moves.tkinter_messagebox.showerror(None, "Failed to save config file.")
            else:
                six.moves.tkinter_messagebox.showinfo(None, "Save successful:\n{}".format(self.cfg_file_name.get()))
    ...
    def menu_saveas(self):
        if self.root_element is None:
            ...
            fname = six.moves.tkinter_filedialog.asksaveasfilename()
            if len(fname) > 0:  # file was selected
                try:
