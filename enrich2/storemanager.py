...
class StoreManager(object):
    ...
    (:py:class:`~enrich2.seqlib.seqlib.SeqLib`, :py:class:`~enrich2.selection.selection.Selection`, and 
    ...
    def labels(self):
        ...
        If the object has no children (such as a SeqLib), the object's _labels are returned.
        """
    ...
    def force_recalculate(self):
        ...
        if self._force_recalculate is None:
            if self.parent is not None:
                ...
                raise ValueError("Forced recalculation option not specified at root [{}]".format(self.name))
        else:
    ...
    def force_recalculate(self, value):
        ...
        if value in (True, False):
            ...
            raise ValueError("Invalid setting '{}' for force_recalculate [{}]".format(value, self.name))
    ...
    def component_outliers(self):
        ...
        if self._component_outliers is None:
            if self.parent is not None:
                ...
                raise ValueError("Calculate component outliers option not specified at root [{}]".format(self.name))
        else:
    ...
    def component_outliers(self, value):
        ...
        if value in (True, False):
            ...
            raise ValueError("Invalid setting '{}' for component_outliers [{}]".format(value, self.name))
    ...
    def plots_requested(self):
        ...
        if self._plots_requested is None:
            if self.parent is not None:
                ...
                raise ValueError("Make plots option not specified at root [{}]".format(self.name))
        else:
    ...
    def plots_requested(self, value):
        ...
        if value in (True, False):
            ...
            raise ValueError("Invalid setting '{}' for plots_requested [{}]".format(value, self.name))
    ...
    def tsv_requested(self):
        ...
        if self._tsv_requested is None:
            if self.parent is not None:
                ...
                raise ValueError("Write tsv option not specified at root [{}]".format(self.name))
        else:
    ...
    def tsv_requested(self, value):
        ...
        if value in (True, False):
            ...
            raise ValueError("Invalid setting '{}' for tsv_requested [{}]".format(value, self.name))
    ...
    def scoring_method(self):
        ...
        if self._scoring_method is None:
            if self.parent is not None:
                ...
                raise ValueError("Scoring method not specified at root [{}]".format(self.name))
        else:
    ...
    def output_dir(self):
        ...
        if self._output_dir is None:
            if self.parent is not None:
                ...
                raise ValueError("No output directory specified at top level [{}]".format(self.name))
        else:
    ...
    def output_dir_override(self):
        ...
        if self._output_dir_override is None:
            if self.parent is not None:
                ...
                raise ValueError("Output directory override not specified at root [{}]".format(self.name))
        else:
    ...
    def output_dir_override(self, value):
        ...
        if value in (True, False):
            ...
            raise ValueError("Invalid setting '{}' for output_dir_override [{}]".format(value, self.name))
    ...
    def plot_dir(self):
        ...
        if self._plot_dir is None:
            dirname = os.path.join(self.output_dir, "plots", "{}_{}".format(fix_filename(self.name), self.store_suffix))
            try:
                if not os.path.exists(dirname):
                    os.makedirs(dirname)
            except OSError as e:
    ...
    def tsv_dir(self):
        ...
        if self._tsv_dir is None:
            dirname = os.path.join(self.output_dir, "tsv",  "{}_{}".format(fix_filename(self.name), self.store_suffix))
            try:
                if not os.path.exists(dirname):
                    os.makedirs(dirname)
            except OSError as e:
    ...
    def logr_method(self):
        ...
        if self._logr_method is None:
            if self.parent is not None:
                ...
                raise AttributeError("Log-ratio method not specified for root element [{}]".format(self.name))
        else:
        ...
        try:
            ...
            if 'output directory' in cfg:
                if self.output_dir_override:
                    logging.warning("Using command line supplied output directory instead of config file output directory", extra={'oname' : self.name})
                else:
                    self.output_dir = cfg['output directory']
            if 'store' in cfg:
                self.store_cfg = True
                if not os.path.exists(cfg['store']):
                    raise IOError('Specified store file "{}" not found'.format(cfg['store']), self.name)
                elif os.path.splitext(cfg['store'])[-1].lower() != ".h5":
                    raise ValueError('Unrecognized store file extension for "{}"'.format(cfg['store']), self.name)
                else:
                    self.store_path = cfg['store']
                    logging.info('Using specified HDF5 data store "{}"'.format(self.store_path), extra={'oname' : self.name})
            else:
    ...
    def store_open(self, children=False, force_delete=True):
        """
        Open the HDF5 file associated with this object. If the ``force_recalculate`` is selected and 
        ``force_delete`` is ``True``, the existing tables under ``'/main'`` will be deleted upon opening.
        ...
        if self.has_store:
            if not self.store_cfg:
                self.store_path = os.path.join(self.output_dir, "{}_{}.h5".format(fix_filename(self.name), self.store_suffix))
                if os.path.exists(self.store_path):
                    logging.info('Found existing HDF5 data store "{}"'.format(self.store_path), extra={'oname' : self.name})
                else:
                    logging.info('Creating new HDF5 data store "{}"'.format(self.store_path), extra={'oname' : self.name})
            self.store = pd.HDFStore(self.store_path)
            if self.force_recalculate and force_delete:
                if "/main" in self.store:
                    logging.info("Deleting existing calculated values", extra={'oname' : self.name})
                    self.store.remove("/main")
                else:
                    logging.warning("No existing calculated values in file", extra={'oname' : self.name})
    ...
    def get_metadata(self, key, store=None):
        ...
        *store* can be an external open HDFStore (used when copying metadata from raw counts). If it is ``None``, use this object's store.
        ...
        try:
            ...
            if store is self.store: # store parameter was None
                raise AttributeError("Invalid HDF store node '{}' [{}]".format(key,
                                     self.name))
            else:
                raise AttributeError("Invalid external HDF store node '{}' in '{}' [{}]".format(key,
                                     store.filename, self.name))
    ...
    def check_store(self, key):
        """
        Checks to see if a particular data frame in the HDF5 store already exists.
    ...
    def map_table(self, source, destination, source_query=None, row_callback=None, row_callback_args=None, destination_data_columns=None):
        """
        ...
        # turn the single table name into a list, so we can use select_as_multiple
        ...
        for df in self.store.select_as_multiple(source, source_query, chunksize=self.chunksize, selector=source[0]):
            ...
            if destination not in self.store:
                ...
                self.store.append(destination, df, min_itemsize={'index' : max_index_length}, data_columns=destination_data_columns)
            else:
